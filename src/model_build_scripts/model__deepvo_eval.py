...
def parse_args():
    ...
    argparser.add_argument('trained_model_file', help="path from where the trained model weights should be loaded.")
...
def create_model(layernames, shape):
    ...
    for layername in layernames:
        # input layers expect subsequenced data: (1, 1, imH, imW, imC)
        input_layers[layername] = tf.keras.layers.Input(batch_size=1, shape=(1, shape[0], shape[1], shape[2]), name=layername)
    ...
    # conv_6: (1, 1, shape[0]/64, shape[1]/64, 1024) -> (1, 1, shape[0]/64 * shape[1]/64 * 1024)
    ...
    lstm1 = LSTM(1000, time_major=False, return_sequences=True, stateful=True, name='LSTM1')(flatten)
    # add second LSTM layer with 1000 units
    lstm2 = LSTM(1000, time_major=False, return_sequences=False, stateful=True, name='LSTM2')(lstm1)

    ## add final dense layer that maps LSTM outputs to 6 output neurons
    out = Dense(6, activation=linear, name='output')(lstm2)
    # NOTE output shape: (1, 6)

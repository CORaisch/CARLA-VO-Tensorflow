...
def create_model(layernames, image_shape):
    ...
    # stack inputs at channel dimension of images
    input_stacked = tf.keras.layers.concatenate(inputs=input_layers, axis=-1, name='stack_inputs')
    # add convolution layers
    conv1 = tf.keras.layers.Conv2D(1024, 7, padding='same', strides=(16,64), activation='relu', data_format='channels_last', name='conv1')(input_stacked)
    conv2 = tf.keras.layers.Conv2D(512, 7, padding='same', strides=8, activation='relu', data_format='channels_last', name='conv2')(conv1)
    conv3 = tf.keras.layers.Conv2D(64, 7, padding='same', strides=2, activation='relu', data_format='channels_last', name='conv3')(conv2)
    # flatten output tensor
    flatten = tf.keras.layers.Flatten(data_format='channels_last', name='flatten')(conv3)
    # make mlp with 6 outpus for the 6 dof poses
    mid = tf.keras.layers.Dense(500, activation='relu', name='dense1')(flatten)
    out = tf.keras.layers.Dense(6, activation='linear', name='dense2')(mid)
